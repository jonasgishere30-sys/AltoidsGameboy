...
struct SnakeGame : Game {
  static const int CS = 12, COLS = 20, ROWS = 24, OY = 32;
  ...
  void begin() override {
    ...
    for (int i = 0; i < len; i++) { bx[i] = 8 - i; by[i] = 12; }
    placeFood();
  }
  ...
  void draw(Canvas& g, uint32_t now) override {
    ...
    for (int y = 0; y < ROWS; y++) for (int x = 0; x < COLS; x++)
      g.drawPixel(x * CS + CS / 2, OY + y * CS + CS / 2, C_LINE);
    float pulse = 0.5f + 0.5f * sinf(now * 0.008f);
    g.fillCircle(fx * CS + 6, OY + fy * CS + 6, 4 + ir(pulse), C_RED);
    for (int i = len - 1; i >= 0; i--) {
      uint16_t c = blend(C_GREEN, rgb(20, 90, 50), (float)i / imax(len, 8));
      rrect(g, bx[i] * CS + 1, OY + by[i] * CS + 1, CS - 2, CS - 2, 3, c);
    }
    // eyes on the head, facing the direction of travel
    int hx = bx[0] * CS + 6, hy = OY + by[0] * CS + 6;
